...
///
/// In [TimerPickerEntryMode.dial] mode, a clock dial is displayed and the user
...
class _TimePickerModel extends InheritedModel<_TimePickerAspect> {
  ...
  final TimerPickerEntryMode entryMode;
  ...
  static TimerPickerEntryMode entryModeOf(BuildContext context) =>
...
class _HourMinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (_TimePickerModel.entryModeOf(context)) {
      case TimerPickerEntryMode.dial:
      case TimerPickerEntryMode.dialOnly:
        height = defaultTheme.hourMinuteSize.height;
      case TimerPickerEntryMode.input:
      case TimerPickerEntryMode.inputOnly:
...
class _HourMinuteTextFieldState extends State<_HourMinuteTextField> with RestorationMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final Diagnosticable inputDecorationTheme =
...
typedef EntryModeChangeCallback = void Function(TimerPickerEntryMode mode);
...
class TimePickerDialog extends StatefulWidget {
  ...
  const TimePickerDialog({
    ...
    this.restorationId,
    this.initialEntryMode = TimerPickerEntryMode.dial,
    this.orientation,
  ...
  final ValueChanged<TimeOfDay>? onNeutral;
  ...
  final TimerPickerEntryMode initialEntryMode;
...
class _TimePickerDialogState extends State<TimePickerDialog> with RestorationMixin {
  late final RestorableEnum<TimerPickerEntryMode> _entryMode = RestorableEnum<TimerPickerEntryMode>(
    widget.initialEntryMode,
    values: TimerPickerEntryMode.values,
  );
  ...
  void _handleEntryModeChanged(TimerPickerEntryMode value) {
  ...
  void _toggleEntryMode() {
    switch (_entryMode.value) {
      case TimerPickerEntryMode.dial:
        _handleEntryModeChanged(TimerPickerEntryMode.input);
      case TimerPickerEntryMode.input:
        _handleEntryModeChanged(TimerPickerEntryMode.dial);
      case TimerPickerEntryMode.dialOnly:
      case TimerPickerEntryMode.inputOnly:
  ...
  void _handleOk() {
    if (_entryMode.value == TimerPickerEntryMode.input ||
        _entryMode.value == TimerPickerEntryMode.inputOnly) {
    ...
    Console.of.log("HANDLE_NEUTRAL");
    if (widget.onNeutral == null) Navigator.pop(context, _selectedTime.value);
    if (widget.onNeutral != null) Navigator.pop(context);
    widget.onNeutral?.call(_selectedTime.value);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Widget actions = Padding(
      ...
      child: Row(
        children: <Widget>[
          if (_entryMode.value == TimerPickerEntryMode.dial ||
              _entryMode.value == TimerPickerEntryMode.input)
            IconButton(
              ...
              icon:
              _entryMode.value == TimerPickerEntryMode.dial
          ...
          Expanded(
            child: Row(
              ...
              children: <Widget>[
                ...
                if ((widget.neutralText?.length ?? 0) > 0)
                  Expanded(
                    child: TextButton(
                      style: pickerTheme.confirmButtonStyle ?? defaultTheme.confirmButtonStyle,
                      onPressed: _handleOk,
                      child: Text(widget.neutralText ?? localizations.okButtonLabel, maxLines: 1, overflow: TextOverflow.ellipsis),
                    ),
                  ),
              ],
            ),
...
class _TimePicker extends StatefulWidget {
  ...
  const _TimePicker({
    ...
    this.minuteLabelText,
    this.restorationId,
    this.entryMode = TimerPickerEntryMode.dial,
    this.orientation,
    this.onEntryModeChanged,
    this.switchToInputEntryModeIcon,
    this.switchToTimerEntryModeIcon,
  ...
  /// when the [entryMode] is [TimerPickerEntryMode.dial], and
  /// [MaterialLocalizations.timePickerInputHelpText] when the [entryMode] is
  /// [TimerPickerEntryMode.input].
  final String? helpText;
...
class _TimePickerState extends State<_TimePicker> with RestorationMixin {
  ...
  void _setEntryMode(TimerPickerEntryMode mode) {
  ...
  void _handleEntryModeToggle() {
    setState(() {
      TimerPickerEntryMode newMode = widget.entryMode;
      switch (widget.entryMode) {
        case TimerPickerEntryMode.dial:
          newMode = TimerPickerEntryMode.input;
        case TimerPickerEntryMode.input:
          _autofocusHour.value = false;
          _autofocusMinute.value = false;
          newMode = TimerPickerEntryMode.dial;
        case TimerPickerEntryMode.dialOnly:
        case TimerPickerEntryMode.inputOnly:
...
/// the input method icon that is shown when the [TimerPickerEntryMode]
/// is [TimerPickerEntryMode.dial].
///
...
Future<TimeOfDay?> showTimerPicker({
  ...
  TimerPickerEntryMode initialEntryMode = TimerPickerEntryMode.dial,
  String? cancelText,
  String? confirmText,
  String? neutralText,
  String? helpText,
  ...
  ValueChanged<TimeOfDay>? onNeutral,
}) async {
...
class _TimePickerDefaultsM2 extends _TimePickerDefaults {
  ...
  BorderSide get dayPeriodBorderSide {
    return BorderSide(
      color: Color.alphaBlend(_colors.onSurface.withValues(alpha: .38), _colors.surface),
    );
  }
  ...
  Color get dayPeriodColor {
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.selected)) {
        return _colors.primary.withValues(alpha:  _colors.brightness == Brightness.dark ? 0.24 : 0.12);
      }
  ...
  Color get dayPeriodTextColor {
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      return states.contains(WidgetState.selected)
          ? _colors.primary
          : _colors.onSurface.withValues(alpha: .60);
    });
  }
  ...
  Color get hourMinuteColor {
    return WidgetStateColor.resolveWith((Set<WidgetState> states) {
      return states.contains(WidgetState.selected)
          ...
          : _colors.onSurface.withValues(alpha: .12);
    });
  }
  ...
  InputDecorationThemeData get inputDecorationTheme {
    return InputDecorationThemeData(
      ...
      hintStyle: hourMinuteTextStyle.copyWith(color: _colors.onSurface.withValues(alpha: .36)),
...
class _TimePickerDefaultsM3 extends _TimePickerDefaults {
  _TimePickerDefaultsM3(this.context, { this.entryMode = TimerPickerEntryMode.dial });
  ...
  TextStyle get hourMinuteTextStyle {
    return WidgetStateTextStyle.resolveWith((Set<WidgetState> states) {
      ...
      return switch (entryMode) {
        TimerPickerEntryMode.dial || TimerPickerEntryMode.dialOnly
        => _textTheme.displayLarge!.copyWith(color: _hourMinuteTextColor.resolve(states)),
        TimerPickerEntryMode.input || TimerPickerEntryMode.inputOnly
