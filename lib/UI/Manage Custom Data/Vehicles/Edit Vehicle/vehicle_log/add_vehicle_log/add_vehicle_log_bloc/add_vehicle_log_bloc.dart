...
class AddVehicleLogBloc extends Bloc<AddVehicleLogEvent, AddVehicleLogState> {
  ...
  Future<Map<String, dynamic>?> _uploadLog({Map<String, dynamic>? body, Map<String, String?>? files}) async => await _aPiRepository.uploadExpenseLogs(infusedFiles: files, body: body);
  ...
  void _onRecordVideoEvent(AddVehicleLogRecordVideoEvent event, Emitter<AddVehicleLogState> emit) async {
    video = await _pickVideo();
    emit(AddVehicleLogCommonState());
  }

  void _onRecordAudioEvent(AddVehicleLogRecordAudioEvent event, Emitter<AddVehicleLogState> emit) async {
    emit(AddVehicleLogRecorderAudioState());
  }

  void _onUploadImageEvent(AddVehicleLogUploadImageEvent event, Emitter<AddVehicleLogState> emit) async {
    image = await _pickImage();
    emit(AddVehicleLogCommonState());
  }

  void _onSubmitEvent(AddVehicleLogSubmitEvent event, Emitter<AddVehicleLogState> emit) async {
    try {
      if ((video == null) && (audio == null) && (image == null) && (notesController.text.trim().isNullOrEmpty)) {
      ...
      if (response != null) {
        ...
        emit(AddVehicleLogSuccessState(response));
      } else {
  ...
  void _onInitialEvent(AddVehicleLogInitialEvent event, Emitter<AddVehicleLogState> emit) {
    vin = event.vin;
    emit(AddVehicleLogCommonState());
  }

  void _onAudioInsertEvent(AddVehicleLogAudioInsertEvent event, Emitter<AddVehicleLogState> emit) {
    audio = event.file;
    emit(AddVehicleLogCommonState());
  }
}