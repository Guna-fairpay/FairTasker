...
class PrivateRenalCheckBloc extends Bloc<PrivateRentalCheckEvent,PrivateRentalCheckState> {
  ...
  Future<Map<String, dynamic>?> _updateToDo(Map<String, dynamic> body, dynamic toDoId) async => await apiRepository.updateToDo(body: body, toDoId: toDoId);
  Future<Map<String, dynamic>?> _addToDo(Map<String, dynamic> body) async => await apiRepository.addToDo(body: body);
  Future<Map<String, dynamic>?> _completeToDo(Map<String, dynamic> body, dynamic toDoId) async => await apiRepository.completeTodo(body: body, todoId: toDoId);
  Future<Map<String, dynamic>?> _deleteToDo(dynamic toDoId, dynamic reason) async => await apiRepository.deleteTodo(id: toDoId, reason: reason);
  ...
  PrivateRenalCheckBloc() : super(PrivateRentalCheckLoadingState()) {
    on<PrivateRentalCheckInitialEvent>(_onInitialEvent);
    on<RentalCheckEvent>(_onCheckEvent);
    on<PrivateRentalCheckSubmitEvent>(_onSubmitEvent);
    on<PrivateRentalCheckCompleteEvent>(_onCompleteEvent);
    on<PrivateRentalCheckDeleteEvent>(_onDeleteEvent);
  }
  ...
  void _onInitialEvent(PrivateRentalCheckInitialEvent event, Emitter<PrivateRentalCheckState> emit) async {
    _editToDoModel = event.todoData;
    emit(PrivateRentalCheckLoadingState());
    var response = await apiRepository.getPrivateRentalCheck();
    _todos = await apiRepository.todo();
    checkLists = response?['data'];
    _processData();
    // apiResponse = apiResponse.map((e)=> e..putIfAbsent('controller', () => TextEditingController())).toList();

    emit(PrivateRentalCheckCommonState());
  }
  void _onSubmitEvent(PrivateRentalCheckSubmitEvent event, Emitter<PrivateRentalCheckState> emit) async {
    try {
      emit(PrivateRentalCheckLoadingState());
      ...
      if (response != null) {
        ...
        emit(PrivateRentalCheckSuccessState());
      } else {
        emit(PrivateRentalCheckCommonState());
      }
    } catch (e) {
  ...
  void _onCompleteEvent(PrivateRentalCheckCompleteEvent event, Emitter<PrivateRentalCheckState> emit) async {
    try {
  ...
  void _onDeleteEvent(PrivateRentalCheckDeleteEvent event, Emitter<PrivateRentalCheckState> emit) async {
    if (event.reason.toString().isNullOrEmpty) {
      emit(PrivateRentalCheckDialogState(model: event.model));
      return;
    }
    try {
      emit(PrivateRentalCheckLoadingState());
  ...
  void _onCheckEvent(RentalCheckEvent event, Emitter<PrivateRentalCheckState> emit) {
    if ((event.model['fix_task'] != null) && (event.model['checked'] == false)) {
      emit(PrivateRentalCheckPopupState(model: event.model));
      return;
    }
