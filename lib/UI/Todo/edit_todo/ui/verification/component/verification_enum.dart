...
enum VerificationInsuranceEnum{
  information_needed,
  confirmed,
  pending_action,
  pending_admin_action,
...
List<String> verificationInsuranceStatus = [
  'Info needed',
  'Confirmed',
  'Pending Action',
  'Pending Action',
...
extension VerificationInsuranceStatusExtension on String?{
  VerificationInsuranceEnum? get statusType => VerificationInsuranceEnum.values.firstWhereOrNull((element) => element.name == (this ?? "unknown"));
  String get statusLabel {
    final input = isNullOrEmpty ? "unknown" : this;
    final index = VerificationInsuranceEnum.values.indexWhere(
            (element) => element.name == input);
    if (index == -1 || index >= verificationInsuranceStatus.length) {
      return 'Unknown';
    }
    return verificationInsuranceStatus[index];
  }
}

extension VerificationInsuranceStatusExtensionByType on VerificationInsuranceEnum {
  Color get statusColor => switch(this) {
    VerificationInsuranceEnum.information_needed => AppC.appColor,
    VerificationInsuranceEnum.confirmed => AppC.green,
    VerificationInsuranceEnum.pending_action => Colors.amber,
    VerificationInsuranceEnum.pending_admin_action =>Colors.amber,
    VerificationInsuranceEnum.unknown => AppC.blue,
    _=> AppC.blue
  };
}