                        ...
                        SizedBox(
                          ...
                          child: Checkbox(
                            ...
                            onChanged: (value) => context
                                ...
                                .add(IsAllMaintenanceCheckEvent(value ?? false)),
                          ),
                        ),
                        Utils.getText(
                    ...
                    ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Column(
                          ...
                          children: [
                            Utils.getText(
                              "${maintenanceCheckListData['name'] ?? ''}".trim(),
                              weight: FontWeight.bold,
                            ),
                            for (var item in checkList) ...[
                              Column(
                                children: [
                                          ...
                                          Expanded(
                                            ...
                                            Utils.dropdownBox(
                                              ...
                                              List<Map<String, dynamic>>.from(item['children'])
                                                ..addAll([
                                                  {"id": 99, "name": "Other"},
                                                  {"id": m.Random().nextInt(99), "name": "Not Checked"}
                                                ]),
                                                  (value) {
                                  ...
                                  Builder(builder: (_) {
                                    final isDropdownList = state.dropdownValue is List;
                                    final isAnyGood = isDropdownList
                                        ? (state.dropdownValue as List).any((e) => e['name'] == 'Good')
                                        : false;

                                    final isInInitialDropdown = state.initialDropDown?.any(
                                          (e) => e['itemId'] == item['id'],
                                    ) ??
                                        false;

                                    final isInChildrenInitial = List.from(item['children']).any(
                                          (element) =>
                                      state.initialDropDown?.map((e) => e['id'].toString()).contains(
                                        element['id'].toString(),
                                      ) ??
                                          false,
                                    );

                                    final isNotGoodSelected = (isDropdownList &&
                                        (state.dropdownValue as List).any((element) =>
                                        element['name'].toString().toLowerCase().trim() != "good")) ||
                                        (state.selectedDropdownValues[item['id']]?.toString().toLowerCase() !=
                                            "good");
                                            ...
                                            Utils.getAddFilledButton(
                                              'Create Task',
                                                ...
                                                context.read<MaintenanceBloc>().add(
                                                  createFixTaskEvent(
                                                    maintenanceTaskId:
                                                    '${maintenanceCheckListData['id']}-${item['id']}-${item['children'].where((e) =>
