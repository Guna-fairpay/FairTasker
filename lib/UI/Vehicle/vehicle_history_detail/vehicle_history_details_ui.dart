...
class VehicleHistoryDetailsUiDialog {
  ...
  static void show(BuildContext context) async {
    ...
    await showDialog(context: context, builder: (context) => VehicleHistoryDetailsUi(bloc : bloc), barrierDismissible: true, useSafeArea: true, useRootNavigator: true);
  }
}

class VehicleHistoryDetailsUi extends StatelessWidget {
  final VehicleHistoryBloc bloc;
  const VehicleHistoryDetailsUi({super.key, required this.bloc});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<VehicleHistoryBloc, VehicleHistoryState>(
      ...
      var mapData = state.selectedTask;
      return Scaffold(
        appBar: AppBar(
          backgroundColor: (mapData['status'].toString().toLowerCase() == "completed") ? AppC.darkGreen : AppC.appColor,
          foregroundColor: Colors.white,
          leading: IconButton(onPressed: ()=> context.popDialog(), icon: Icon(Icons.clear_rounded)),
          leadingWidth: 30,
          ...
          actions: [
            Transform.scale(
              ...
              child: Switch(value: (mapData['status'].toString().toLowerCase() == "completed"),
                onChanged: (value) => bloc.add(VehicleHistoryCompleteEvent(mapData['id'], value))),
            )
          ],
        ),
        body: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                if ((mapData['expense_attachment'] != null) &&
                  ...
                  GridView.builder(
                    ...
                    itemBuilder: (context, index) => Container(
                        ...
                        child: ImageViewer(
                            fit: BoxFit.cover,
                            imageInput: Str.STORAGE_BASE_URL+(jsonDecode(
                                mapData['expense_attachment'].toString()))
                                .values
                                .elementAt(index))),
                  )
              ],
              if (mapData['reservation_no'] != null)
                ListWidget(
                    item: "Reservation No - ${mapData['reservation_no'] ?? ""}"),
            ],
          ),
        ),
      );
