...
class Authenticator {
  ...
  Future<void> getBearerToken() async {
    ...
    log('getBearerToken.response : $response');
    if (response != null && (response['status'] == 200) && response['data'].toString().isNotEmpty) {
      Session.of.set(Str.frBearerToken, response['data'].toString());
      Utils.setStringPreference(Str.frBearerToken, response['data'].toString());
    }
  }
